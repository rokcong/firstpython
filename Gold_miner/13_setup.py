...
class Claw(pygame.sprite.Sprite):
    ...
    def update(self, to_x):
        ...
        self.offset.x += to_x
        self.rotate()   # 회전처리
        # print(self.angle, self.direction)     // 각도 계산 출력
        # rect_center = self.position + self.offset
        # self.rect = self.image.get_rect(center = rect_center)

    def rotate(self):
        # pygame.transform.rotate     // -> 화면상 매끄럽지 않음
        self.image = pygame.transform.rotozoom(self.original_image, -self.angle, 1) # 회전 대상 이미지, 회전 각도, 이미지 크기(scale) 정의
...
class Gemstone(pygame.sprite.Sprite):
    ...
    def set_position(self, position, angle):
        ...
        rad_angle = math.radians(angle)     # 세타 값
...
def setup_gemstone():
    ...
    ston_price, stone_speed = 10, 2
    diamond_price, diamond_speed = 600, 7

    # 작은 금
    small_gold = Gemstone(gemstone_images[0], (200, 380),small_gold_price, small_gold_speed)   # 0 번째 이미지를 (200, 300) 위치에
    ...
    # 돌
    gemstone_group.add(Gemstone(gemstone_images[2], (300,380), ston_price, stone_speed))
    gemstone_group.add(Gemstone(gemstone_images[2], (700,330), ston_price, stone_speed))
    gemstone_group.add(Gemstone(gemstone_images[2], (1000,480), ston_price, stone_speed))
    # 다이아몬드
...
to_x = 0    # x 좌표 기준으로 집게 이미지응 이동시킬 값 저장 변수
...
return_speed = 20 # 아무것도 없이 돌아올 때 이동 스프디
...
claw = Claw(claw_image, (screen_width // 2, 110))   # 가로 위치는 화면 가로 크기 기준으로 절반, 세로위치는 위에서  110px
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:    # 마우스 버튼 누를 때 집게를 뻗음
            if claw.direction != STOP:    # 조건 추가: 집게가 좌우로 이동 중일 때만 마우스 이벤트 처리
                claw.set_direction(STOP)    # 죄우 멈춤
    ...
    if claw.offset.x < default_offset_x_claw:   # 원위치에 오면
        ...
        if caught_gemstone:     # 잡힌 보석이 있다면
            update_score(caught_gemstone.price)   # 접수 업데이트 처리 (나중에)
    ...
    if not caught_gemstone:     # 잡힌 보석이 없다면 충돌 체크
        for gemstone in gemstone_group:
            ...
            if pygame.sprite.collide_mask(claw, gemstone):  # 투명 영역은 제외하고 실제 이미지 영역에 대한 충돌 처리
