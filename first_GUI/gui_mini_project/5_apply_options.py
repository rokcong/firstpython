...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
                                        ...
                                        initialdir=r"C:\Users") # 최초에 사용자 지정한 경로로 보여줌
    # 사용자가 선택한 파일 목록
    for file in files:
...
def browse_dest_path():
    foler_selected = filedialog.askdirectory()
    if foler_selected == '':  # 사용자가 취소를 누를 때
        print("폴더 선택 취소")
        return
    # print(foler_selected)
    txt_dest_path.delete(0, END)
    txt_dest_path.insert(0, foler_selected)
...
def merge_image():
    ...
    try:
        ...
        image_sizes = []    # (width1, height1), (width1, height2), ...
        if img_width > -1:
            # width 값 변경
        ...
        # 계산식
        # 100 * 60 이미지 있음 -> width를 80으로 줄이면 height는?
        ...
        result_img = Image.new("RGB", (max_width, total_height), (255, 255, 255)) # background => white
        y_offset = 0    # y 위치
        ...
        # 포맷 옵션 처리
        file_name = "JD_photo." + img_format
        dest_path = os.path.join(txt_dest_path.get(), file_name)
        result_img.save(dest_path)
        msgbox.showinfo("알림", "작압이 완료되었습니다")
...
# 가로 넓이 옵션
# 가로 넓이 레이블
lbl_width = Label(frame_option, text="가로 넓이", width=9)
...
# 간격 옵션
# 간격 옵션 레이블
lbl_space = Label(frame_option, text="간격", width=9)
...
# 파일 포맷 옵션
# 파일 포맷 옵션 레이블
lbl_format = Label(frame_option, text="포맷", width=9)
